...
final class NativeQueueCoordinator {
    ...
    func start() {
        processingQueue.async { [weak self] in
            ...
            print("🚀 NativeQueueCoordinator started")
    ...
    func stop() {
        processingQueue.async { [weak self] in
            ...
            print("🛑 NativeQueueCoordinator stopped")
        }
    }
    ...
    func pause() {
        processingQueue.async { [weak self] in
            self?.isPaused = true
            print("⏸️ NativeQueueCoordinator paused")
        }
    }
    ...
    func resume() {
        processingQueue.async { [weak self] in
            ...
            self.isPaused = false
            print("▶️ NativeQueueCoordinator resumed")
    ...
    func notifyJobsAvailable(count: Int, queue: String) {
        processingQueue.async { [weak self] in
            ...
            print("📬 Jobs available: \(count) on queue '\(queue)'")
    ...
    private func checkForJobs() {
        ...
        makeRequest(path: "/_native/queue/status") { [weak self] result in
                ...
                if let json = try? JSONSerialization.jsonObject(with: data) as? [String: Any],
                   let pending = json["pending"] as? Int {
                    ...
                    if let config = json["config"] as? [String: Any] {
                        ...
                        print("📋 Queue config applied: minDelay=\(self.minDelayBetweenJobs)s, batchSize=\(self.maxJobsPerBatch), pollInterval=\(self.emptyQueuePollInterval)s")
                    }
            ...
            case .failure(let error):
                print("❌ Queue status check failed: \(error.localizedDescription)")
                // Retry after a delay
                self.scheduleNextJob(delay: self.emptyQueuePollInterval)
            }
        }
    ...
    private func processNextJob() {
        guard isRunning, !isPaused else {
            if isPaused {
                print("⏸️ Job processing paused, waiting...")
            }
            return
        }
        ...
        if maxJobsPerBatch > 0 && processedInBatch >= maxJobsPerBatch {
            print("📦 Batch limit reached (\(maxJobsPerBatch)), yielding...")
            processedInBatch = 0
        ...
        makeRequest(path: "/_native/queue/work", method: "POST") { [weak self] result in
            ...
            case .failure(let error):
                print("❌ Job processing failed: \(error.localizedDescription)")
                self.scheduleNextJob(delay: self.emptyQueuePollInterval)
            }
        }
    }
    ...
    private func handleWorkResponse(_ data: Data) {
        ...
        if processed {
            ...
            if let error = json["error"] as? [String: Any] {
                ...
                print("⚠️ Job \(jobId) failed: \(errorMessage) (\(durationMs)ms)")
                ...
                print("✅ Job \(jobId) completed: \(jobName) (\(durationMs)ms)")
            ...
            if pending > 0 {
                ...
                print("🎉 Queue empty! Processed \(totalProcessed) total jobs")
            ...
            print("📭 Queue empty (reason: \(reason)), polling in \(emptyQueuePollInterval)s")
            scheduleNextJob(delay: emptyQueuePollInterval)
        }
    }
    ...
    private func makeRequest(
        ...
        // Use the internal PHP scheme handler by making a request through the app
        let urlString = "php://127.0.0.1\(path)"
        
        guard let url = URL(string: urlString) else {
            completion(.failure(NSError(domain: "NativeQueueCoordinator", code: -1, 
