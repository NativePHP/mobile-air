...
final class NativePHPBootstrap: @unchecked Sendable {
    ...
    private func bootRuntime() {
        ...
        // 8. Defer the extra PHP runtimes — start AFTER the critical path
        //    completes so they don't compete for CPU/memory during first page
        //    render, and only once the app is on screen.
        //
        //    Both lanes boot their own TSRM contexts on top of the persistent
        //    runtime: one for the queue worker, four for the async pool (each a
        //    full Laravel boot). Paying for that during a headless background
        //    wake is what gets the process jetsammed mid-task, and the wake
        //    never asked for it.
        //
        //    Jobs dispatched by background work still queue normally — they're
        //    picked up when the app is next opened. AsyncTask::dispatch() returns
        //    false while the pool is down, which is the honest answer: its
        //    completion callback wakes the UI runloop, and a headless launch
        //    hasn't got one.
        if booted {
