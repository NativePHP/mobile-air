...
- Use `native:icon` for iconography (SF Symbols on iOS, Material Icons on Android — cross-platform names like
  ...
  asks for them. Prefer the **typed icon enums** (`Ios`, `Android`, `AndroidOutlined`) bound with `:ios` /
  `:android` over string names — they're autocompletable and can't misspell a symbol. The enum catalogs are
  generated by the `nativephp/mobile-ui` package; see its guidelines for generating and importing them.
